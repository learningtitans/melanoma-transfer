...
def load_module(mod):
    ...
    #return importlib.import_module('.'+mod.split('.py')[0])
...
def get_submission_filename_fold(fold):                                         # <-- Adapted to generate filenames including the fold
    sha = get_commit_sha()
    return "data/sub_{}_{}_{}.csv".format(sha,
                                       datetime.now().replace(microsecond=0),
                                       fold)
