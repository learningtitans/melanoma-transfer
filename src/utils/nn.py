...
class Net(NeuralNet):
    ...
    def __init__(self, *args, **kwargs):      # This function works for blend.py
        self.eval_size = kwargs['eval_size']
        super(Net, self).__init__(*args, **kwargs)
    ...
    def train_loop(self, X, y, epochs=None):
        ...
        while epoch < epochs:
            ...
            for Xb, yb in self.batch_iterator_train(X_train, y_train, seed=epoch):
                if not self.regression: yb = yb.reshape((yb.shape[0],))			# <--- need to change the shape for running classification
                #if epoch == 1:
                #    for i in range(len(Xb)):
                #        hash = random.getrandbits(128)
                #        randString = "%032x"%hash
                #        im = Image.fromarray(Xb[i].transpose(1,2,0).astype('uint8'))
                #        randString = str(np.random.normal(0,1,1))
                #        im.save("images/"+randString+".jpg")
            ...
            if self.custom_scores and self.eval_size:

                y_true = np.concatenate(y_true)
                y_pred = np.concatenate(y_pred)
                y_pred = np.clip(y_pred, np.min(y_true), np.max(y_true))
                avg_custom_scores = self.custom_scores[0][1](y_true, y_pred)
            ...
            if self.custom_scores and self.eval_size:
                info[self.custom_scores[0][0]] = avg_custom_scores
